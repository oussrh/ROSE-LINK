...
if [ -d "$PROJECT_DIR/backend" ]; then
    ...
    cp -r "$PROJECT_DIR/backend"/*.py "$DEB_DIR/opt/rose-link/backend/" 2>/dev/null || true
    cp -r "$PROJECT_DIR/backend/requirements*.txt" "$DEB_DIR/opt/rose-link/backend/" 2>/dev/null || true
