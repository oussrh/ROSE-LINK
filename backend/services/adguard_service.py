...
class AdGuardService:
    ...
    async def get_status(cls) -> AdGuardStatus:
        ...
        ret, _, _ = CommandRunner.systemctl_is_active(ADGUARD_SERVICE_NAME)
        status.running = ret == 0
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{ADGUARD_API_URL}/control/protection",
                    ...
                    if resp.status == 200:
                        ...
                        logger.error(f"Failed to set protection: HTTP {resp.status}")
                        return False
        except Exception as e:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        logger.error(f"Failed to set filtering: HTTP {resp.status}")
                        return False
        except Exception as e:
