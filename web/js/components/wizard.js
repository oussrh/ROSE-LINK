...
export function skipWizard() {
    if (confirm(t('wizard_skip_confirm'))) {
        markWizardCompleted();
        hideWizard();
    }
}
        ...
        <div class="py-4">
            ...
            <div class="bg-gray-700/50 rounded-lg p-4 mb-4">
                ...
                <div class="flex gap-2">
                    <input type="file" id="wizard-vpn-file" accept=".conf"
                           class="flex-1 bg-gray-600 rounded px-3 py-2 text-sm file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:bg-rose-600 file:text-white file:cursor-pointer">
                    <button id="wizard-vpn-import" class="px-4 py-2 bg-green-600 hover:bg-green-700 rounded text-sm transition-smooth flex items-center gap-1">
...
async function importVpnProfile() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const data = await response.json();
            statusEl.innerHTML = `<span class="text-red-400">${data.detail || t('upload_failed')}</span>`;
        }
    } catch {
