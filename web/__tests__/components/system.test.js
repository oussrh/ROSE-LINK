...
import { renderSystemInfo, initVPNSettingsForm, updateRebootConfirmations } from '../../js/components/system.js';
...
describe('System Component', () => {
    beforeEach(() => {
        document.body.innerHTML = `
            ...
            <button hx-confirm="Are you sure?">Reboot</button>
            <button hx-confirm="Are you sure?">Shutdown</button>
        `;
        jest.clearAllMocks();
    });
    ...
    describe('updateRebootConfirmations', () => {
        it('should update all hx-confirm attributes', () => {
            updateRebootConfirmations();

            const buttons = document.querySelectorAll('[hx-confirm]');
            buttons.forEach(btn => {
                expect(btn.getAttribute('hx-confirm')).toBe('confirm_reboot');
            });
        });

        it('should call t() for translation', () => {
            updateRebootConfirmations();

            expect(t).toHaveBeenCalledWith('confirm_reboot');
        });

        it('should handle no elements with hx-confirm', () => {
            document.body.innerHTML = '<button>No confirm</button>';

            expect(() => updateRebootConfirmations()).not.toThrow();
        });
    });
});
